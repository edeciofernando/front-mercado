...
const ResumoProdutos = () => {
  ...
  const obterDados = async () => {
    try {
      const dadosResumo = await inAxios.get("produtos/resumo");
      setResumo(dadosResumo.data);

      const dadosGrafico = await inAxios.get("produtos/marcas");

      // cria um array e adiciona a primeira linha
      const arrayGrafico = [["Marca", "R$ Total"]];

      // percorre cada linha do JSON e adiciona ao array
      dadosGrafico.data.map((dado) =>
        arrayGrafico.push([dado.marca, dado.total])
      );
    ...
    <div className="container">
      ...
      <div className="d-flex justify-content-center">
        <Chart
          ...
          data={grafico}
          options={{
            title: "Total do Estoque de Produtos - por Marca",
            chartArea: { width: "80%" },
            hAxis: { title: "Marca do Produto" },
            vAxis: { title: "Preço Total R$" },
